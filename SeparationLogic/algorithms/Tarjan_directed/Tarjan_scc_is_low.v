...
Section IS_LOW.
  ...
  Proof.
    intro_state. destruct H as [Hinv [Hna Hndone_a]].
    unfold low_forset_inv in Hinv.
    destruct Hinv as [Hsiv [Hinv' [Hvalid [Hfa_vis [Huvis Hmin]]]]].
    apply Hoare_conj.
    - (* low_forset_inv u done *)
      unfold low_forset_inv. apply Hoare_conj.
      + (* stack_in_visited *)
        eapply Hoare_conseq_pre. 2: apply (preloop_keep_stack_in_visited a).
        intros s1 Hs1. subst s1. exact Hsiv.
      + apply Hoare_conj.
        * (* dfn_inv *)
          eapply Hoare_conseq_pre. 2: apply (preloop_keep_dfn_inv a).
          intros s1 Hs1. subst s1. exact Hinv'.
        * apply Hoare_conj.
          -- (* dfn_valid *)
             eapply Hoare_conseq_post with
               (Q1 := fun _ s => dfn_valid g s root)
               (Q2 := fun _ s => a ∈ visited s /\ dfn_valid g s root /\ dfn_inv s).
             { intros _ s1 [Hvis1 [Hvalid1 _]]. exact Hvalid1. }
             eapply Hoare_conseq_pre. 2: apply preloop_preserves_dfn_valid.
             intros s1 Hs1. subst s1. split; [exact Hna | split; [exact Hvalid | split; [exact Hinv' | exact Hfa_vis]]].
          -- apply Hoare_conj.
             ++ (* fa_visited *)
                eapply Hoare_conseq_pre. 2: apply (preloop_keep_fa_visited a).
                intros s1 Hs1. subst s1. exact Hfa_vis.
             ++ apply Hoare_conj.
                ** (* u ∈ visited *)
                ...
                ** (* min condition *)
                   ...
                   assert (Hchild_eq: children_done s u done == children_done s0 u done).
                   { unfold children_done. apply Sets_equiv_Sets_included. split; sets_unfold;
                       intros x [Hx_done [Hx_fa Hx_neq]]; repeat split; auto. }
                   assert (Hback_eq: back_edges_done s u done == back_edges_done s0 u done).
                   { unfold back_edges_done. apply Sets_equiv_Sets_included. split; sets_unfold;
                       intros x [Hx_done [Hx_stack Hx_fa]]; repeat split; auto.
                     - destruct Hx_stack as [Heq | Hin]; [subst x; exfalso; apply Hndone_a; exact Hx_done | exact Hin].
                     - right. exact Hx_stack. }
                   assert (Hlow_child: forall w, w ∈ children_done s0 u done -> low s w = low s0 w).
                   { intros w0 Hw0. unfold children_done in Hw0. sets_unfold in Hw0.
                     destruct Hw0 as [Hw0_done [Hw0_fa Hw0_neq]].
                     unfold s. simpl. unfold equiv_decb. destruct (equiv_dec w0 a) as [Heq | Hneq].
                     - exfalso. rewrite Heq in Hw0_done. apply Hndone_a. exact Hw0_done.
                     - reflexivity. }
                   assert (Hlow_back: forall w, w ∈ back_edges_done s0 u done -> low s w = low s0 w).
                   { intros w0 Hw0. unfold back_edges_done in Hw0. sets_unfold in Hw0.
                     destruct Hw0 as [Hw0_done [Hw0_stack Hw0_fa]].
                     ...
                     - reflexivity. }
                   assert (Hlow_u: low s u = low s0 u).
                   { unfold s. simpl. unfold equiv_decb. destruct (equiv_dec u a) as [Heq | Hneq].
                     - exfalso. apply Hna. rewrite <- Heq. exact Huvis.
                     - reflexivity. }
                   assert (Hlow_all: forall w, children_done s0 u done w \/ back_edges_done s0 u done w \/ w = u -> low s w = low s0 w).
                   { intros w0 [Hw0 | [Hw0 | Hw0]].
                     - apply Hlow_child. exact Hw0.
                           ...
                           *** left. destruct Ha1_L as [w [[Hw_in Hw_min] Heq_a1]]. exists w. split.
                               ---- unfold min_object_of_subset. split.
                                    ...
                                    ++++ intros x Hx. apply Hchild_eq in Hx.
                                         ...
                                         rewrite (Hlow_all x (or_introl Hx)).
                                         apply Hw_min. exact Hx.
                               ---- unfold s. destruct (equiv_dec w a) as [Heq_wa | Hneq_wa].
                                    { exfalso. unfold children_done in Hw_in. sets_unfold in Hw_in.
                                      destruct Hw_in as [Hw_done _]. rewrite Heq_wa in Hw_done. apply Hndone_a. exact Hw_done. }
                                    simpl. unfold equiv_decb. destruct (equiv_dec w a) as [Heq | Hneq].
                                    { exfalso. unfold children_done in Hw_in. sets_unfold in Hw_in.
                                      destruct Hw_in as [Hw_done _]. rewrite Heq in Hw_done. apply Hndone_a. exact Hw_done. }
                                    { exact Heq_a1. }
                           *** right. destruct Ha1_R as [w [[Hw_in Hw_min] Heq_a1]].
                               assert (Hdfn_w: dfn s w = dfn s0 w).
                               { unfold s. destruct (equiv_dec w a) as [Heq_wa | Hneq_wa].
                                 - exfalso. destruct Hw_in as [Hw_back | Hw_u].
                                   + unfold back_edges_done in Hw_back. sets_unfold in Hw_back.
                                     destruct Hw_back as [Hw_done _]. rewrite Heq_wa in Hw_done.
                                     apply Hndone_a. exact Hw_done.
                                   + subst w. exfalso. apply Hna. rewrite <- Heq_wa. exact Huvis.
                                 - unfold s. simpl. unfold equiv_decb. destruct (equiv_dec w a) as [Heq | Hneq].
                                   + contradiction.
                                   + reflexivity. }
                               assert (Hdfn_u: dfn s u = dfn s0 u).
                               { unfold s. simpl. unfold equiv_decb. destruct (equiv_dec u a) as [Heq_ua | Hneq_ua].
                                 - exfalso. apply Hna. rewrite <- Heq_ua. exact Huvis.
                                 - reflexivity. }
                               exists w. split.
                               ---- unfold min_object_of_subset. split.
                                    ++++ destruct Hw_in as [Hw_back | Hw_u].
                                    ...
                                    ++++ intros x Hx. destruct Hx as [Hx_back | Hx_u].
                                         { assert (Hdfn_x: dfn s x = dfn s0 x).
                                           { unfold s. destruct (equiv_dec x a) as [Heq_xa | Hneq_xa].
                                             - exfalso. unfold back_edges_done in Hx_back. sets_unfold in Hx_back.
                                               destruct Hx_back as [Hx_done _]. rewrite Heq_xa in Hx_done.
                                               apply Hndone_a. exact Hx_done.
                                             - unfold s. simpl. unfold equiv_decb. destruct (equiv_dec x a) as [Heq | Hneq].
                                               + contradiction.
                                               + reflexivity. }
                                           rewrite Hdfn_w. rewrite Hdfn_x. apply Hw_min. left. apply Hback_eq. exact Hx_back. }
                                         { subst x. rewrite Hdfn_w. rewrite Hdfn_u. apply Hw_min. right. reflexivity. }
                               ---- unfold s. destruct (equiv_dec w a) as [Heq_wa | Hneq_wa].
                                    ...
                                    { exfalso. destruct Hw_in as [Hw_back | Hw_u].
                                      - unfold back_edges_done in Hw_back. sets_unfold in Hw_back.
                                        destruct Hw_back as [Hw_done _]. rewrite Heq in Hw_done. apply Hndone_a. exact Hw_done.
                                      - subst w. exfalso. apply Hna. rewrite <- Heq. exact Huvis. }
                                    { exact Heq_a1. }
                       +++ apply Nat.le_refl.
                   --- intros a2 Ha2. exists a2. split.
                       +++ destruct Ha2 as [Ha2_L | Ha2_R].
  ...
  Proof.
    intros Hneq Hndone.
    apply Hoare_conj with (Q1 := fun _ s => low_forset_inv u done s) (Q2 := fun _ s => fa s v = u /\ done_visited done s).
    - (* Part A: low_forset_inv *)
      apply Hoare_conseq_post with (Q2 := fun _ s => low_forset_inv u done s /\ done_visited done s).
      { intros _ s [Hinv _]. exact Hinv. }
      apply Hoare_conseq_pre with
        (P2 := fun s => low_forset_inv u done s /\ ~ v ∈ visited s /\ ~ done v /\ done_visited done s).
      { intros s [Hinv [Hfa [Hnv [Hnd Hdv]]]]. exact (conj Hinv (conj Hnv (conj Hnd Hdv))). }
      unfold tarjan_scc. hoare_fix_nolv_auto V. intros W IH a. unfold tarjan_scc_f.
      (* IH: forall x, Hoare (low /\ ~x in visited /\ ~done x /\ done_visited) (W x) (low /\ done_visited) *)
      apply (Hoare_bind (fun s => low_forset_inv u done s /\ ~ a ∈ visited s /\ ~ done a /\ done_visited done s) (preloop a) (fun _ s => low_forset_inv u done s /\ a ∈ visited s /\ ~ done a /\ done_visited done s) (fun _ => forset (fun v0 : V => dg_step g a v0) (process_edge a W) ;; If (fun s : SCCSt => low s a = dfn s a) (pop_scc a)) (fun _ s => low_forset_inv u done s /\ done_visited done s)).
      { (* preloop a *)
        assert (Hpre_nd: Hoare (fun s => ~ done a) (preloop a) (fun _ s => ~ done a)) by
          (unfold preloop; unfold_op; intro_state; hoare_auto_s; destruct H; exact H).
        assert (Hpre_dv: Hoare (fun s => done_visited done s) (preloop a) (fun _ s => done_visited done s)) by
          (unfold preloop; unfold_op; intro_state; hoare_auto_s; subst s; simpl;
           unfold done_visited in *; intros w Hw; apply H in Hw; sets_unfold; left; exact Hw).
        assert (Hpre_vis: Hoare (fun s => True) (preloop a) (fun _ s => a ∈ visited s)) by
          (unfold preloop; unfold_op; intro_state; hoare_auto_s; subst s; simpl; sets_unfold; right; reflexivity).
        apply Hoare_conseq_post with (Q2 := fun _ s => (low_forset_inv u done s /\ a ∈ visited s) /\ (~ done a /\ done_visited done s)).
        { intros _ s [[Hinv Hav] [Hnd Hdv]]. split. exact Hinv. split. exact Hav. split. exact Hnd. exact Hdv. }
        apply Hoare_conj with (Q1 := fun _ s => low_forset_inv u done s /\ a ∈ visited s) (Q2 := fun _ s => ~ done a /\ done_visited done s).
        - apply Hoare_conseq_pre with (P2 := fun s => low_forset_inv u done s /\ ~ a ∈ visited s /\ ~ done a).
          { intros s [Hinv [Hnv [Hnd Hdv]]]. exact (conj Hinv (conj Hnv Hnd)). }
          apply (preloop_keeps_low_forset_inv_other u a done).
        - apply Hoare_conj with (Q1 := fun _ s => ~ done a) (Q2 := fun _ s => done_visited done s).
          + apply Hoare_conseq_pre with (P2 := fun s => ~ done a). { intros s [Hinv [Hnv [Hnd Hdv]]]. exact Hnd. } apply Hpre_nd.
          + apply Hoare_conseq_pre with (P2 := fun s => done_visited done s). { intros s [Hinv [Hnv [Hnd Hdv]]]. exact Hdv. } apply Hpre_dv. }
      { simpl. intros _. intro_state. destruct H as [Hinv' [Hav' [Hnd_a Hdv']]].
  ...
  Proof.
    intros Hinv.
    unfold low_forset_inv, scc_low_valid_v in *.
    destruct Hinv as [Hsiv [Hdinv [Hdvalid [Hfav [Huvis Hmin]]]]].
    set (universe := fun v : V => dg_step g u v) in *.
    eapply min_eq_forward; [typeclasses eauto | exact Hmin | | ].
    - (* forward: old min values are <= new min values *)
      intros a1 Ha1. exists a1. split.
      { destruct Ha1 as [Ha1L | Ha1R].
        + (* children_done side *)
          destruct Ha1L as [w [[Hw_in Hw_min] Heq_a1]].
          left. exists w. split.
          { unfold min_object_of_subset. split.
            { unfold children_done, universe in Hw_in.
              destruct Hw_in as [Hdg [Hfa_eq Hfa_ne]].
              eapply state_to_dfs_tree_step_char_backward; eauto.
              eapply Hfav. exact Hfa_eq. }
            { intros x Hx. unfold dg_step, state_to_dfs_tree in Hx. simpl in Hx.
              destruct Hx as [e [Hstep [Hfst_eq Hsnd_eq]]].
              unfold original_step in Hstep. simpl in Hstep.
              destruct Hstep as [v [Hvis [Hfa_ne_v [Hfst_fa Hsnd_v]]]].
              rewrite Hsnd_v in Hsnd_eq. rewrite Hfst_fa in Hfst_eq.
              simpl in Hfst_eq, Hsnd_eq. subst.
              apply Hw_min. unfold children_done, universe.
              split; [| split; [exact Hfst_fa | exact Hfa_ne_v]].
              exact (Huniv v). } }
          exact Heq_a1. }
        + (* back_edges_done + u side *)
          destruct Ha1R as [w [[Hw_in Hw_min] Heq_a1]].
          right. exists w. split.
          { unfold min_object_of_subset. split.
            { destruct Hw_in as [Hw_back | Hw_u].
              - left. unfold scc_back_edge.
                unfold back_edges_done, universe in Hw_back.
                destruct Hw_back as [Hdg [Hstack Hfa_ne]].
                split; [exact Hdg | split; [exact Hstack |]].
                intro Htree. apply state_to_dfs_tree_step_char in Htree.
                destruct Htree as [Hfa_eq' [_ _]].
                apply Hfa_ne. exact Hfa_eq'.
              - right. exact Hw_u. }
            { intros x Hx. destruct Hx as [Hx_back | Hx_u].
              - destruct Hx_back as [[Hdg' [Hstack' Hnot_tree]] | Hx_u'].
                + apply Hw_min. left.
                  unfold back_edges_done, universe.
                  split; [exact Hdg' | split; [exact Hstack' |]].
                  intro Hfa_eq_x. apply Hnot_tree.
                  eapply state_to_dfs_tree_step_char_backward; eauto.
                  * admit. (* fa s x <> x — holds because on stack and not tree root *)
                  * eapply stack_in_visited_inv. exact Hsiv. exact Hstack'.
                + subst. apply Hw_min. right. reflexivity.
              - subst. apply Hw_min. right. reflexivity. } }
          exact Heq_a1. } }
      apply Nat.le_refl.
    - (* backward: symmetric *)
      intros a2 Ha2. exists a2. split.
      { destruct Ha2 as [Ha2L | Ha2R].
        + destruct Ha2L as [w [[Hw_in Hw_min] Heq_a2]].
          left. exists w. split.
          { unfold min_object_of_subset. split.
            { unfold children_done, universe.
              unfold dg_step, state_to_dfs_tree in Hw_in. simpl in Hw_in.
              destruct Hw_in as [e [Hstep [Hfst_eq Hsnd_eq]]].
              unfold original_step in Hstep. simpl in Hstep.
              destruct Hstep as [v [Hvis [Hfa_ne [Hfst_fa Hsnd_v]]]].
              rewrite Hsnd_v in Hsnd_eq. rewrite Hfst_fa in Hfst_eq.
              simpl in Hfst_eq, Hsnd_eq. subst.
              split; [| split; [exact Hfst_fa | exact Hfa_ne]].
              exact (Huniv v). }
            { intros x Hx. unfold children_done, universe in Hx.
              destruct Hx as [Hdg [Hfa_eq Hfa_ne]].
              apply Hw_min. unfold dg_step, state_to_dfs_tree. simpl.
              eapply state_to_dfs_tree_step_char_backward; eauto.
              eapply Hfav. exact Hfa_eq. } }
          exact Heq_a2. }
        + destruct Ha2R as [w [[Hw_in Hw_min] Heq_a2]].
          right. exists w. split.
          { unfold min_object_of_subset. split.
            { destruct Hw_in as [Hw_back | Hw_u].
              - left. unfold back_edges_done, universe.
                destruct Hw_back as [[Hdg' [Hstack' Hnot_tree]] | Hw_u'].
                2: { subst. exact Hw_u. }
                split; [exact Hdg' | split; [exact Hstack' |]].
                intro Hfa_eq_w. apply Hnot_tree.
                eapply state_to_dfs_tree_step_char_backward; eauto.
                * admit. (* fa s w <> w *)
                * eapply stack_in_visited_inv. exact Hsiv. exact Hstack'.
              - right. exact Hw_u. }
            { intros x Hx. destruct Hx as [Hx_back | Hx_u].
              - destruct Hx_back as [Hdg [Hstack Hfa_ne]].
                apply Hw_min. left. left. split; [exact Hdg | split; [exact Hstack |]].
                intro Htree. apply state_to_dfs_tree_step_char in Htree.
                destruct Htree as [Hfa_eq' [_ _]].
                apply Hfa_ne. exact Hfa_eq'.
              - subst. apply Hw_min. right. reflexivity. } }
  ...
  Lemma forset_keep_low_forset_inv (u: V) (W: V -> program (@SCCSt V) unit):
    ...
    Hoare (fun s => low_forset_inv u ∅ s /\ (forall v, fa s v = u -> v = u))
          ...
          (fun _ s => scc_low_valid_v s u /\ dfn_valid g s root /\ dfn_inv s /\ fa_visited s).
  Proof.
    intros HW_low HW_self_vis HW_keep_vis HW_fa_children.
    set (universe := fun v : V => dg_step g u v).
    set (P := fun (done: V -> Prop) (s: SCCSt) =>
      low_forset_inv u done s /\ dfn_valid g s root /\ dfn_inv s /\ fa_visited s).
    eapply Hoare_conseq.
    3: apply (@Hoare_forset SCCSt V P universe (process_edge u W)).
    - (* strengthen precondition: original pre -> P empty *)
      simpl. intros s [Hinv Hfa_u].
      destruct Hinv as [Hsiv [Hdinv0 [Hdvalid0 [Hfav0 [Huvis0 Hmin0]]]]].
      unfold P. simpl. split; [| split; [exact Hdvalid0 | split; [exact Hdinv0 | exact Hfav0]]].
      split; [exact Hsiv | split; [exact Hdinv0 | split; [exact Hdvalid0 | split; [exact Hfav0 | split; [exact Huvis0 | exact Hmin0]]]]].
    - (* weaken postcondition: P universe -> scc_low_valid_v *)
      simpl. intros b s [Hlow [Hdvalid [Hdinv Hfav]]].
      split; [| split; [exact Hdvalid | split; [exact Hdinv | exact Hfav]]].
      apply low_forset_inv_to_scc_low_valid. exact Hlow.
    - (* Proper proof for P *)
      unfold P, Proper, respectful. intros done1 done2 Hequiv s1 s2 Heq. subst s2.
      split; intros [Hlow [Hdvalid [Hdinv Hfav]]]; split; [| split; [exact Hdvalid | split; [exact Hdinv | exact Hfav]]].
      + eapply (low_forset_inv_proper u done1 done2 Hequiv s1 s1). reflexivity. exact Hlow.
      + apply Sets_equiv_Sets_included in Hequiv. destruct Hequiv as [Hincl12 Hincl21].
        eapply (low_forset_inv_proper u done2 done1).
        apply Sets_equiv_Sets_included. split; [exact Hincl21 | exact Hincl12].
        reflexivity. exact Hlow.
    - (* forset step: Hoare (P done) (process_edge u W a) (P (done ∪ [a])) *)
      intros done a Hsub Huniv_a Hnotdone.
      intro_state. destruct H as [[Hsiv [Hdinv0 [Hdvalid0 [Hfav0 [Huvis0 Hmin0]]]]] [Hval [Hdinv' Hfav']]].
      unfold process_edge, if_else. intro_state. apply Hoare_choice.
      + (* Tree edge: ~ a ∈ visited *)
        apply Hoare_assume_bind. simpl. intro_state.
        destruct H2 as [Hnv_a Hs2_eq]. subst s2. subst s1.
        rename s0 into s_pre.
        assert (Hnd_a: ~ done a). { intro Hd. apply Hnv_a. eapply done_visited_visited. apply (conj Hsiv (conj Hdinv0 (conj Hdvalid0 (conj Hfav0 (conj Huvis0 Hmin0))))). exact Hd. }
        (* set_fa a u *)
        unfold set_fa. intro_state. hoare_auto_s. subst s0.
        eapply Hoare_bind with (R := fun _ s => P (done ∪ [a]) s).
        { (* W a *)
          apply Hoare_conseq_pre with (P2 := fun s => low_forset_inv u done s /\ ~ a ∈ visited s /\ dfn_valid g s root /\ dfn_inv s /\ fa_visited s).
          { intros st Heq. subst st. simpl.
            split. { apply (set_fa_preserves_low_forset_inv_for_new_child u u a done s_pre Hnv_a Huvis0 Huvis0 Hnd_a).
              split; [exact Hsiv | split; [exact Hdinv0 | split; [exact Hdvalid0 | split; [exact Hfav0 | split; [exact Huvis0 | exact Hmin0]]]]]. }
            split; [exact Hnv_a | split; [exact Hval | split; [exact Hdinv' | exact Hfav']]]. }
          admit. (* W a preserves low_forset_inv u done + invariants *) }
        { simpl. intros _. intro_state. hoare_auto_s. destruct H as [Hinv_done [Hnv_a' [Hval' [Hdinv'' Hfav'']]]].
          unfold update_low. intro_state. hoare_auto_s.
          - unfold set_low. intro_state. hoare_auto_s. subst s1. subst s.
            unfold P. simpl.
            assert (Hmin_eq: low s0 a = Nat.min (low s0 u) (low s0 a)). { symmetry. apply Nat.min_r. lia. }
            split. { rewrite Hmin_eq. apply (update_low_preserves_low_forset_inv_for_other u u (low s0 a) (done ∪ [a]) s0).
              { intro Hda. apply Hnv_a'. eapply done_visited_visited. exact Hinv_done. exact Hda. }
              exact Hinv_done. }
            split; [exact Hval' | split; [exact Hdinv'' | exact Hfav'']].
          - destruct H1. subst s. unfold P. simpl. split; [exact Hinv_done | split; [exact Hval' | split; [exact Hdinv'' | exact Hfav'']]]. }
