...
Proof.
  intros timer_p_pre fin_pre vis1_pre radj_row_pre radj_col_pre n_pre u_pre
        X_low_level_spec radj_row_l_low_level_spec radj_col_l_low_level_spec
        g_low_level_spec hi lo timer_m i vis1_m fin_m
        PreH1 PreH2 PreH3 PreH4 PreH5 PreH6 PreH7 PreH8 PreH9 PreH10 PreH11 PreH12 PreH13 PreH14 PreH15.
  pre_process. entailer!. cbv [Znth nth Z.to_nat] in *.
  assert (Hhige : (csr_hi u_pre radj_row_l_low_level_spec <= i)%Z).
  { rewrite PreH7 in PreH1. lia. }
  pose proof (dfs_finish_from_exit g_low_level_spec radj_col_l_low_level_spec
                        radj_row_l_low_level_spec u_pre i Hhige) as Hexit.
  assert (Hscc : safeExec (pre_dfs1 g_low_level_spec radj_col_l_low_level_spec
                            radj_row_l_low_level_spec vis1_m fin_m timer_m)
            (assertS (fun st => (timer st < length (bijective_listV g_low_level_spec))%nat);;
             (t <- get (fun st t => t = timer st) ;; set_finish u_pre t))
            X_low_level_spec).
  { eapply safeExec_proequiv with (c1 := dfs_finish_from g_low_level_spec
                                          radj_col_l_low_level_spec
                                          radj_row_l_low_level_spec u_pre i).
    - exact Hexit.
    - exact PreH5. }
  apply safeExec_assertS_seq in Hscc.
  destruct Hscc as [sigma [[Hbnd Hpre] _]].
  unfold pre_dfs1 in Hpre.
  destruct Hpre as [Hpv [Hpf Hpt]].
  assert (Hlen : Datatypes.length (bijective_listV g_low_level_spec) = Z.to_nat (adj_verts g_low_level_spec)).
  { assert (Hg : AdjGraphValid g_low_level_spec).
    { destruct PreH2 as [C1 [C2 [C3 [C4 [C5 [C6 [C7 [C8 [C9 C10]]]]]]]]]. exact C1. }
    pose proof (AdjGraph_bijective_listV_perm g_low_level_spec Hg) as Hp.
    pose proof (Permutation_length Hp) as Hplen.
    rewrite length_map, length_seq in Hplen. exact Hplen. }
  rewrite Hlen, Hpt in Hbnd.
  rewrite PreH4 in Hbnd.
  assert (Hn0 : (0 <= n_pre)%Z) by lia.
  assert (Htm0 : (0 <= timer_m)%Z) by lia.
  apply (Z2Nat.inj_lt timer_m n_pre Htm0 Hn0) in Hbnd.
  lia.
Qed.
...
Proof.
  ...
  all: pose proof PreH1 as Hwf;
       ...
       | apply (dfs1_entry_close g_low_level_spec radj_col_l_low_level_spec
         ...
         [ exact PreH1 | exact PreH2 | exact Hub | exact PreH4 ]
...
Proof. right; intros; pre_process. Exists timer_m_2. entailer!. apply dfs1_skip_close. - rewrite <- PreH7; exact PreH10. - match goal with H : csr_wf1 ?g ?rc ?rr ?vis ?fin |- _ => destruct H as [C1 [C2 [C3 [C4 [C5 [C6 [C7 [C8 [C9 C10]]]]]]]]] end; assert (Hb : (0 <= i < m_of radj_row_l_low_level_spec)%Z) by lia; pose proof (C8 i Hb) as Hc; rewrite <- PreH18 in *; destruct Hc as [Hc1 Hc2]; lia. - match goal with H : ?v = Znth ?i ?fc 0 |- _ => rewrite <- H; match goal with H2 : Znth ?v ?vis 0 <> 0 |- _ => exact H2 end end. - match goal with H : safeExec (pre_dfs1 ?g ?rc ?rr ?vis ?fin ?tv) (dfs_finish_from ?g ?rc ?rr ?u ?i) ?X |- _ => exact H end. Qed.
...
Proof.
  ...
  apply (dfs1_return_close g_low_level_spec radj_col_l_low_level_spec radj_row_l_low_level_spec u_pre i vis1_m fin_m timer_m X_low_level_spec).
  all: first
       [ split; [ exact PreH12 | rewrite PreH4; exact PreH13 ]
       | exact PreH15
       | exact C4
       | pose proof PreH1 as H1; rewrite PreH7 in H1; lia
       | exact PreH5
       | unfold csr_wf1 in *; repeat (rewrite Zlength_replace_Znth in * |- *);
         repeat (split; try assumption; try lia; try congruence) ].
Qed.
...
Proof. intros; pre_process. entailer!. rewrite PreH30. apply dfs1_recurse_close. - rewrite <- PreH19; exact PreH22. - match goal with H : csr_wf1 ?g ?rc ?rr ?vis ?fin |- _ => destruct H as [C1 [C2 [C3 [C4 [C5 [C6 [C7 [C8 [C9 C10]]]]]]]]] end; assert (Hb : (0 <= i < m_of radj_row_l_low_level_spec)%Z) by lia; pose proof (C8 i Hb) as Hc; rewrite <- PreH30 in *; destruct Hc as [Hc1 Hc2]; lia. - match goal with H : ?v = Znth ?i ?fc 0 |- _ => rewrite <- H; match goal with H2 : Znth ?v ?vis 0 = 0 |- _ => exact H2 end end. - match goal with H : safeExec (pre_dfs1 ?g ?rc ?rr ?vis ?fin ?tv) (dfs_finish_from ?g ?rc ?rr ?u ?i) ?X |- _ => exact H end. Qed.
...
Proof. right. intros timer_p_pre fin_pre vis1_pre radj_row_pre radj_col_pre n_pre u_pre X_low_level_spec radj_row_l_low_level_spec radj_col_l_low_level_spec g_low_level_spec vis1_m fin_m timer_m i lo hi v PreH1 PreH2 PreH3 PreH4 PreH5 PreH6 PreH7 PreH8 PreH9 PreH10 PreH11 PreH12 PreH13 PreH14 PreH15 PreH16 PreH17 PreH18 PreH19 PreH20 PreH21 PreH22 PreH23 PreH24 PreH25 PreH26 PreH27 PreH28 PreH29 PreH30. split. - apply (proof_of_dfs1_partial_solve_wit_6_pure_split_goal_1 timer_p_pre fin_pre vis1_pre radj_row_pre radj_col_pre n_pre u_pre X_low_level_spec radj_row_l_low_level_spec radj_col_l_low_level_spec g_low_level_spec vis1_m fin_m timer_m i lo hi v PreH1 PreH2 PreH3 PreH4 PreH5 PreH6 PreH7 PreH8 PreH9 PreH10 PreH11 PreH12 PreH13 PreH14 PreH15 PreH16 PreH17 PreH18 PreH19 PreH20 PreH21 PreH22 PreH23 PreH24 PreH25 PreH26 PreH27 PreH28 PreH29 PreH30); exact H. - apply (proof_of_dfs1_partial_solve_wit_6_pure_split_goal_2 timer_p_pre fin_pre vis1_pre radj_row_pre radj_col_pre n_pre u_pre X_low_level_spec radj_row_l_low_level_spec radj_col_l_low_level_spec g_low_level_spec vis1_m fin_m timer_m i lo hi v PreH1 PreH2 PreH3 PreH4 PreH5 PreH6 PreH7 PreH8 PreH9 PreH10 PreH11 PreH12 PreH13 PreH14 PreH15 PreH16 PreH17 PreH18 PreH19 PreH20 PreH21 PreH22 PreH23 PreH24 PreH25 PreH26 PreH27 PreH28 PreH29 PreH30); exact H. Qed.
